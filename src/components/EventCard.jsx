...
export default function EventCard({ event, onClick }) {
  return (
    <button
      ...
      className="group w-full text-left bg-[#111111] border border-white/[0.07] rounded-2xl overflow-hidden hover:border-white/20 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2"
      style={{ '--tw-ring-color': event.color.accent }}
      aria-label={`View details for ${event.title}`}
    >
      {/* Banner */}
      <div
        className="h-44 relative overflow-hidden"
        style={{ background: `linear-gradient(145deg, ${event.color.from} 0%, ${event.color.to} 100%)` }}
      >
        {/* SVG pattern */}
        <svg className="absolute inset-0 w-full h-full opacity-[0.09]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id={`p-${event.id}`} x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
        ...
        {/* Glow blob */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28 rounded-full blur-3xl transition-all duration-300 group-hover:scale-125"
        ...
        >
          <div
            ...
            style={{ background: `${event.color.accent}15`, borderColor: `${event.color.accent}30` }}
          >
        ...
        <div className="absolute top-3 right-3">
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-black/40 text-white backdrop-blur-sm">
            {event.price}
          </span>
        </div>
      </div>
      ...
      {/* Card body */}
      <div className="p-5">
        <h3 className="text-white font-bold text-base leading-tight mb-3 group-hover:text-gray-100 transition-colors">
          {event.title}
        </h3>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2 text-gray-500 text-xs">
            ...
            <span>{event.date} · {event.time.split('–')[0].trim()}</span>
          </div>
          <div className="flex items-center gap-2 text-gray-500 text-xs">
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
