...
export default function TicketCard({ formData }) {
  ...
  const handleMouseMove = (e) => {
    ...
    setTilt({ rotateX: -y * 8, rotateY: x * 8, brightness: 1 + Math.abs(x) * 0.05 })
  }

  const handleMouseLeave = () => setTilt({ rotateX: 0, rotateY: 0, brightness: 1 })
  ...
  return (
    <div className="perspective-[1200px] cursor-pointer select-none" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
      <article
        ref={cardRef}
      ...
      >
          ...
          <svg className="absolute inset-0 w-full h-full opacity-[0.06]" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="ticket-squiggly" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
                <path d="M0 25 Q6.25 15 12.5 25 Q18.75 35 25 25 Q31.25 15 37.5 25 Q43.75 35 50 25" fill="none" stroke="#f97316" strokeWidth="1.5"/>
                <path d="M0 45 Q6.25 35 12.5 45 Q18.75 55 25 45 Q31.25 35 37.5 45 Q43.75 55 50 45" fill="none" stroke="#f97316" strokeWidth="1.5"/>
                <path d="M0 5 Q6.25 -5 12.5 5 Q18.75 15 25 5 Q31.25 -5 37.5 5 Q43.75 15 50 5" fill="none" stroke="#f97316" strokeWidth="1.5"/>
              </pattern>
            </defs>
        ...
        <div className="relative z-10 p-6 sm:p-8">
          <div className="flex items-start justify-between mb-8 gap-4">
            ...
            <div className="text-[#4a3a5e] font-black text-6xl sm:text-7xl font-sans leading-none select-none"
              aria-hidden="true" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)', letterSpacing: '0.05em' }}>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              {avatarPreview ? (
                <img src={avatarPreview} alt={`${fullName}'s avatar`} className="w-14 h-14 rounded-xl object-cover border-2 border-orange-400/30 shrink-0" />
              ) : (
                <div className="w-14 h-14 rounded-xl bg-orange-500/20 border-2 border-orange-400/30 flex items-center justify-center shrink-0">
                  <span className="text-orange-400 text-xl font-bold">{fullName.charAt(0).toUpperCase()}</span>
                </div>
              )}
              <div>
                <p className="text-white text-lg font-bold leading-tight">{fullName}</p>
                <a href={`https://github.com/${github.replace('@', '')}`} target="_blank" rel="noopener noreferrer"
      ...
      <p className="text-center text-[#7a6e8a] text-xs mt-4" aria-live="polite">Move your mouse over the ticket to see the 3D effect</p>
      <div className="flex justify-center mt-5">
        ...
        >
          {downloading ? (
            <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4l3-3-3-3v4a8 8 0 00-8 8h4z"/></svg>Generating PDF…</>
          ) : (
            <><svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd"/></svg>Download Ticket PDF</>
          )}
        </button>
      </div>
    </div>
